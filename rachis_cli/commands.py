...
import rachis_cli.builtin.dev
import rachis_cli.builtin.info
import rachis_cli.builtin.tools

from rachis_cli.click.command import BaseCommandMixin
...
class RootCommand(BaseCommandMixin, click.MultiCommand):
    ...
    _builtin_commands = {
        'info': rachis_cli.builtin.info.info,
        'tools': rachis_cli.builtin.tools.tools,
        'dev': rachis_cli.builtin.dev.dev
    }
    ...
    def _plugin_lookup(self):
        ...
        # See note in `rachis_cli.completion.write_bash_completion_script`
        # for why `self._plugins` will not always be obtained from
        # `rachis_cli.cache.CACHE.plugins`.
        if self._plugins is None:
            import rachis_cli.core.cache
            self._plugins = rachis_cli.core.cache.CACHE.plugins

        name_map = {}
        for name, plugin in self._plugins.items():
            if plugin['actions']:
                name_map[rachis_cli.util.to_cli_name(name)] = plugin
        return name_map
    ...
    def get_command(self, ctx, name):
        ...
        try:
            ...
            from rachis_cli.util import get_close_matches
...
class PluginCommand(BaseCommandMixin, click.MultiCommand):
    ...
    def __init__(self, plugin, name, *args, **kwargs):
        ...
        params = [
            ...
            rachis_cli.util.example_data_option(self._get_plugin),
            rachis_cli.util.citations_option(self._get_citation_records)
        ]
    ...
    def _get_version(self, ctx, param, value):
        ...
        import rachis_cli.util
        pm = rachis_cli.util.get_plugin_manager()
...
class ActionCommand(BaseCommandMixin, click.Command):
    ...
    def __init__(self, name, plugin, action):
        ...
        self._misc = [
            click.Option(['--output-dir'],
                         type=rachis_cli.click.type.OutDirType(),
        ...
        self._misc.extend([
            rachis_cli.util.example_data_option(
        ...
        if self.action['deprecated']:
            ...
            mig_help = self._format_migration_help(
                rachis_cli.util.to_cli_name(self.plugin['name']), mig_dict
            )
    ...
    def _build_generated_options(self):
        import rachis_cli.click.option
    ...
    def __call__(self, **kwargs):
        ...
        if output_dir:
            ...
            if potential_cache and os.path.exists(potential_cache) and \
                ...
                rachis_cli.util.exit_with_error(exc)
        ...
        if action.deprecated:
            ...
            msg = ('Plugin warning from %s:\n\n%s is deprecated and '
                   ...
                   (rachis_cli.util.to_cli_name(self.plugin['name']),
                    self.name))
            click.echo(CONFIG.cfg_style('warning', msg))
        ...
        try:
            ...
            if verbose:
                # log is not a file
                log = 'stderr'
            rachis_cli.util.exit_with_error(e, header=header, traceback=log)
        else:

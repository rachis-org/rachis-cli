...
@dev.command(name='refresh-cache',
             ...
             help="Refresh the CLI cache. Use this command if you are "
                  "developing a plugin, or rachis_cli itself, and want your "
...
def refresh_cache():
    ...
    rachis_cli.core.cache.CACHE.refresh()


import_theme_help = \
    ("Allows for customization of rachis-cli's command line styling based on "
     "an imported .theme (INI formatted) file. If you are unfamiliar with "
     ".ini formatted files look here https://en.wikipedia.org/wiki/INI_file."
     "\n"
     "\n"
...
def import_theme(theme):
    ...
    shutil.copy(theme, os.path.join(rachis_cli.util.get_app_dir(),
                'cli-colors.theme'))
...
def reset_theme():
    ...
    path = os.path.join(rachis_cli.util.get_app_dir(), 'cli-colors.theme')

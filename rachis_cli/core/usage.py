...
class CLIUsage(usage.Usage):
    ...
    def merge_metadata(self, name, *variables):
        ...
        var._rachis_cli_ref = \
            ' '.join(v.to_interface_name() for v in variables)
        return var
    ...
    def get_metadata_column(self, name, column_name, variable):
        ...
        # this is our special "short-circuit" attr to handle special-case
        # .to_interface_name() needs
        var._rachis_cli_ref = (variable.to_interface_name(), column_name)
        return var

    def view_as_metadata(self, name, variable):
        # use the given name so that namespace behaves as expected,
        # then overwrite it because viewing is a no-op in rachis-cli
        ...
        var._rachis_cli_ref = variable.to_interface_name()
        return var
    ...
    def _rename_outputs(self, vars_dict, dir_name):
        for signature_name, variable in vars_dict.items():
            ...
            variable._rachis_cli_ref = os.path.join(dir_name, name)
    ...
    def _make_param(self, value, state):
        ...
        opt = rachis_cli.click.option.GeneratedOption(prefix=type_[0], **state)
        option = opt.opts[0]
...
class ReplayCLIUsage(CLIUsage):
    ...
    def _make_param(self, value: Any, state: Dict) -> List[Tuple]:
        ...
        state : dict
             ...
             See rachis_cli.core.state.py.

        Returns
        -------
        list of tuple
            See rachis_cli.core.usage.CLIUsage._make_param
    ...
    def action(
        ...
        '''
        plugin_name = rachis_cli.util.to_cli_name(action.plugin_id)
        action_name = rachis_cli.util.to_cli_name(action.action_id)

...
# NOTE: This module is closely coupled with `rachis_cli.cache`. It's a separate
# module to avoid cluttering `rachis_cli.cache` with completion script code,
# and provides a place to support other shell completion scripts in the future
# (e.g. zsh).
...
def write_bash_completion_script(plugins, path):
    ...
    # `rachis_cli.commands.RootCommand` could have already asked for the cache,
    # for example, if the user ran a command and the cache must be refreshed.
    # The bash completion script is generated by traversing the `RootCommand`
    # tree, so there is a cycle when `RootCommand` attempts to access the
    # cache in order to build itself. We work around this by bootstrapping
    # the `RootCommand`'s `._plugins` attribute with the plugin state that
    # has already been loaded by `DeploymentCache`.

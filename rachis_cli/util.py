...
def get_cli_command_names():
    """Names of the CLI front-ends installed in this deployment.

    The shell completion script registers one completion function against
    each of these: `rachis` & `qiime`; `mosh` in a moshpit environment
    """
    import importlib.metadata
    return sorted(entrypoint.name for entrypoint in
...
def example_data_option(get_plugin, action_name=None):
    import click
    from rachis_cli.click.type import OutDirType

    def callback(ctx, param, value):
        if not value or ctx.resilient_parsing:
            return
        else:
            import rachis_cli.core.usage as usage
